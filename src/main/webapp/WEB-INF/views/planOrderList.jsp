...
<script type="text/javascript">
	...
	function selectRow(ship_code, p_name, c_name, p_code, c_code, ship_num, ship_date, ship_deli, e_name) {
	    ...
	    row.find('td:eq(7)').html('<input type="date" class="form-control" style="width: 100px; readonly="readonly" name="ship_deli" value="' + ship_deli + '">'); // 납기일자 설정
	    ...
	    <nav>
    	   <ul class="pagination">
	        ...
	        <c:if test="${page.startPage > page.pageLimit}">
		        <li class="page-item">
		          <a class="page-link  mx-1 " href="planOrderList?currentPage=${page.startPage-page.pageLimit}&ship_code=${ship.ship_code}&p_code=${ship.p_code}&c_code=${ship.c_code}&date_from=${ship.date_from}&date_to=${ship.date_to}" aria-label="Previous">
		            	이전
		          </a>
		        </li>
	        </c:if>
	        ...
	        <c:forEach var="i" begin="${page.startPage }" end="${page.endPage }" varStatus="status">
	        	<li id="page${status.index}" class="page-item"><a class="page-link mx-1 text-body" href="planOrderList?currentPage=${i}&ship_code=${ship.ship_code}&p_code=${ship.p_code}&c_code=${ship.c_code}&date_from=${ship.date_from}&date_to=${ship.date_to}">${i }</a></li>
	        </c:forEach>
	        ...
	        <c:if test="${page.endPage < page.totalPage}">
		        <li class="page-item">
		          <a class="page-link mx-1 text-body" href="planOrderList?currentPage=${page.startPage+page.pageLimit}&ship_code=${ship.ship_code}&p_code=${ship.p_code}&c_code=${ship.c_code}&date_from=${ship.date_from}&date_to=${ship.date_to}" aria-label="Next">
		            	다음
		          </a>
		        </li>
	        </c:if>
