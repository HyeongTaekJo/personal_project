...
<script type="text/javascript">
    ...
    function searchWorkOrder() {
        ...
        //alert(sendData);
        location.href = "performanceList?" + sendData;
    }
  ...
  <div class="modal-dialog modal-dialog-scrollable modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalScrollableTitle">수주 목록</h5>
		...
		<div class="col-lg-12">
		    ...
		    <div class="row mt-8">
		    	...
		    	<form id="form1" action="productList">
				    ...
				    <div class="mb-8">
				       <!-- heading -->
				       <h1 class="mb-1">실적 조회</h1>
				       <p>총  ${performanceCnt} 건</p>
				    </div>
				    <!-- 제품조회 -->
				     <div class="row" id="k" style="margin-bottom: 10px;">
					    <div class="col-md-2">
					        <label for="validationCustom01" class="form-label">실적번호</label>
					        <input type="text" class="form-control" id="validationCustom01" name="pm_code" style="max-width: 200px;" placeholder="실적번호 입력" oninput="checkNumber(this);" value="${performance.pm_code}">
					    </div>
					    
					    <!-- 수주 시작일자 -->
					     <div class="col-md-2">
					        <label for="startDate" class="form-label">실적 시작일자</label>
					        <input type="date" class="form-control" id="startDate" name="date_from" value="${performance.date_from}">
					    </div> 
					    <!-- 수주 종료일자 -->
					    <div class="col-md-2">
					        <label for="endDate" class="form-label">실적 종료일자</label>
					        <input type="date" class="form-control" id="endDate" name="date_to" value="${performance.date_to}">
					    </div>
					    ...
					    <div class="col-md-2">
					        ...
					        <select class="form-select" id="validationCustom04" name="p_code" style="max-width: 200px;" >
					        	...
					        	<c:forEach var="productList" items="${productList}">
					        		 <option value="${productList.p_code}" ${productList.p_code eq performance.p_code ? 'selected' : ''}>${productList.p_name}</option>
					    ...
					    <div class="col-md-2">
					        ...
					        <select class="form-select" id="validationCustom04" name="c_code" style="max-width: 200px;">
					        	...
					        	<c:forEach var="customerList" items="${customerList}">
					        		<option value="${customerList.c_code}" ${customerList.c_code eq performance.c_code ? 'selected' : ''}>${customerList.c_name}</option>
					    ...
					    <div class="col-md-2">
					        ...
					        <select class="form-select" id="validationCustom04" name="e_code" style="max-width: 200px;" >
					        	...
					        	<c:forEach var="empList" items="${empList}">
					        		 <option value="${empList.e_code}" ${empList.e_code eq performance.e_code ? 'selected' : ''}>${empList.e_name}</option>
					        		<%-- <option value="${bigList.b_code}">${bigList.b_name}</option> --%>
					        	</c:forEach>
					        </select>
					    </div>
	       ...
	       <form id="form2" action="newProductInsert">
		       <div class="table-responsive" style="text-align: center;">
		          <table id="list" class="table text-nowrap">
		             ...
		             <tbody>
		             	<c:forEach var="performanceList" items="${performanceList }" varStatus="status">
			                <tr id="performance${status.index }">
			                   <td class="align-middle">${startRow}</td>
			                   <td class="align-middle">${performanceList.pm_code}  </td>
			                   <td class="align-middle">${performanceList.wo_code}  </td>
			                   <td class="align-middle">${performanceList.p_name}  </td>
			                   <td class="align-middle">${performanceList.c_name}  </td>
			                   <td class="align-middle">${performanceList.order_qty}  </td>
			                   <td class="align-middle">${performanceList.qty}  </td>
			                   <td class="align-middle">${performanceList.e_name}  </td>
			                   <td class="align-middle">${performanceList.pm_date}  </td>
	    ...
	    <nav>
    	   <ul class="pagination">
	        ...
	        <c:if test="${page.startPage > page.pageLimit}">
		        <li class="page-item">
		          <a class="page-link  mx-1 " href="performanceList?currentPage=${page.startPage-page.pageLimit}&pm_code=${performance.pm_code}&p_code=${performance.p_code}&c_code=${performance.c_code}&e_code=${performance.e_code}&date_from=${performance.date_from}&date_to=${performance.date_to}" aria-label="Previous">
		            	이전
		          </a>
		        </li>
	        </c:if>
	        ...
	        <c:forEach var="i" begin="${page.startPage }" end="${page.endPage }" varStatus="status">
	        	<li id="page${status.index}" class="page-item"><a class="page-link mx-1 text-body" href="performanceList?currentPage=${i}&pm_code=${performance.pm_code}&p_code=${performance.p_code}&c_code=${performance.c_code}&e_code=${performance.e_code}&date_from=${performance.date_from}&date_to=${performance.date_to}">${i }</a></li>
	        </c:forEach>
	        ...
	        <c:if test="${page.endPage < page.totalPage}">
		        <li class="page-item">
		          <a class="page-link mx-1 text-body" href="performanceList?currentPage=${page.startPage+page.pageLimit}&pm_code=${performance.pm_code}&p_code=${performance.p_code}&c_code=${performance.c_code}&e_code=${performance.e_code}&date_from=${performance.date_from}&date_to=${performance.date_to}" aria-label="Next">
		            	다음
		          </a>
		        </li>
	        </c:if>
